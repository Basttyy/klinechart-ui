...
const shortPositionTool: OverlayTemplate = {
  name: 'shortPositionTool',
  totalStep: 4,
  ...
  createPointFigures: ({ coordinates, precision, overlay }) => {
    ...
    if (coordinates.length > 1) {
      ...
      if(coordinates.length > 2) {
        ...
        let target = useOrder().calcTarget(points[0].value!, points[1].value!, precision.price, undefined, 'sell')
        let stop = useOrder().calcStopOrTarget(points[1].value!, points[2].value!, precision.price)
        const tags = [
          `Target: ${target} (${((Number(points[1].value! - points[0].value!)/points[1].value!)*100).toFixed(2)}%) ${(Number(target)*multiplier).toFixed(1)}`, 
          `Open P&L: ${useOrder().calcPL(points[1].value!, precision.price, true, 'sell')}, RRR: ${(Number(target)/Math.abs(Number(stop))).toFixed(1)}`, 
          `Stop: ${stop} (${((Number(points[1].value! - points[2].value!)/points[1].value!)*100).toFixed(2)}%) ${(Number(stop)*multiplier).toFixed(1)}`
        ]
...
export default shortPositionTool
